...
pub trait Visitor {
    fn visit(&mut self, contract: &Contract) {
        match contract {
            ...
            Contract::And(first_contract, second_contract) => self.visit_and(first_contract.as_ref(), second_contract.as_ref()),
        }
    }
    ...
    fn visit_and(&mut self, c_1: &Contract, c_2: &Contract);
}
